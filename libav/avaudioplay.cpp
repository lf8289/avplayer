...
int avaudioplay_start(avaudioplay* audio)
{
	...
	ret = pthread_create(&audio->m_audio_dec_thrd, &attr, audio_dec_thrd,(void*) audio->m_play);
	if (ret)
	{
	...
	ret = pthread_create(&audio->m_audio_render_thrd, &attr, audio_render_thrd, (void*) audio->m_play);
	if (ret)
	{
		printf("ERROR; return code from pthread_create() is %d\n", ret);
		return ret;
