...
void queue_flush(av_queue *q)
{
	...
	if (q->m_type == QUEUE_PACKET)
	{
		...
		for (pkt = (AVPacketList *)q->m_first_pkt; pkt != NULL; pkt = pkt1)
		{
			pkt1 = pkt->next;
	...
	else if (q->m_type == QUEUE_AVFRAME)
	{
		...
		pthread_mutex_lock(&q->m_mutex);
		for (pkt = (AVFrameList *)q->m_first_pkt; pkt != NULL; pkt = pkt1)
		{
			pkt1 = pkt->next;
